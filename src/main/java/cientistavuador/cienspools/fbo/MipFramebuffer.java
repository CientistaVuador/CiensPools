 ...
 * @author Cien
 */
public class MipFramebuffer implements Framebuffer {
    ...
    public MipFramebuffer(int cuts) {
        if (cuts < 1) {
            throw new IllegalArgumentException("Cuts must be at least 1.");
        }
        this.cuts = cuts;
        this.width = 1;
        this.height = 1;
    ...
    public void resize(int width, int height) {
        initialize();
        int divideby = (int) Math.pow(2.0, this.cuts);
        ...
        this.width = width;
        this.height = height;
        updateColorBuffer(this.state.colorBuffer);
    }
